...
class PlannerNode:
    ...
    def get_rotated_rect_points(self, cx, cy, yaw, width, length, resolution):
        """
        Generate discrete points representing the border of a rotated rectangle (vehicle)
        """
        points_x = []
        points_y = []
        ...
        # Generate points along the perimeter in local coordinates
        # We use a slightly finer resolution to ensure no gaps
        step = resolution / 1.5 
        
        # Local perimeter points (counter-clockwise)
        # Front edge (x = hl, y from -hw to hw)
        for ly in np.arange(-hw, hw, step):
            lx = hl
            points_x.append(cx + lx * c - ly * s)
            points_y.append(cy + lx * s + ly * c)
            
        # Left edge (y = hw, x from hl to -hl)
        for lx in np.arange(hl, -hl, -step):
            ly = hw
            points_x.append(cx + lx * c - ly * s)
            points_y.append(cy + lx * s + ly * c)
            
        # Rear edge (x = -hl, y from hw to -hw)
        for ly in np.arange(hw, -hw, -step):
            lx = -hl
            points_x.append(cx + lx * c - ly * s)
            points_y.append(cy + lx * s + ly * c)
            
        # Right edge (y = -hw, x from -hl to hl)
        for lx in np.arange(-hl, hl, step):
            ly = -hw
            points_x.append(cx + lx * c - ly * s)
            points_y.append(cy + lx * s + ly * c)
    ...
    def create_obstacle_map(self):
        """Create obstacle map from environment bounds and static vehicles"""
        ox, oy = [], []
        
        # 1. Create boundary obstacles
        ...
        # 2. Add Static Vehicles as obstacles
        static_vehicles = self.obstacles_config.get('static_vehicles', [])
        
        if static_vehicles:
            rospy.loginfo(f"Processing {len(static_vehicles)} static obstacle vehicles...")
            
            for i, vehicle in enumerate(static_vehicles):
                vx = vehicle['x'] * -1
                vy = vehicle['y']
                vyaw = 3.14 - math.radians(vehicle['yaw'])
                
                # Use class width/length if not specified in individual vehicle param
                v_width = vehicle.get('width', self.width)
                v_length = vehicle.get('length', self.length)
                
                # Get contour points for this vehicle
                v_ox, v_oy = self.get_rotated_rect_points(
                    vx, vy, vyaw, v_width, v_length, self.xy_resolution
                )
                
                ox.extend(v_ox)
                oy.extend(v_oy)
                rospy.logdebug(f"Added obstacle vehicle {i} at ({vx}, {vy})")
        else:
            rospy.loginfo("No static vehicles found in configuration.")

...
# 路径数据容器，包含路径长度、路径类型、x位置、y位置、偏航角等信息
class Path:
    """
    Path data container
    """
...
# 绘制箭头的函数，用于在图中绘制路径的方向。
def plot_arrow(x, y, yaw, length=6, width=1, fc="r", ec="k"):
...
# 生成路径的函数，根据起始姿态和目标姿态，以及最大曲率和步长，生成一系列候选路径。
...
# Reeds-Shepp路径规划的主函数，接受起始姿态和目标姿态等参数，调用generate_path函数生成路径，并选择成本最小的路径作为最终路径。

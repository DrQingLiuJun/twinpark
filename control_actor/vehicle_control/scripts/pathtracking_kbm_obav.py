...
class Vehicle():
    def __init__(
            ...
            obstacle_circles: np.ndarray = np.empty((0, 3)), # [obs_x, obs_y, obs_radius]
    ...
    def append_frame(self, steer: float, accel: float, optimal_traj: np.ndarray, sampled_traj_list: np.ndarray) -> list:
        ...
        if optimal_traj is not None and optimal_traj.size > 0:
        ...
        if sampled_traj_list is not None and sampled_traj_list.size > 0:
            min_alpha_value = 0.15
            max_alpha_value = 0.30
            num_trajs = len(sampled_traj_list)
            for idx, sampled_traj in enumerate(sampled_traj_list):
                # draw darker for better samples (权重高的样本透明度更高)
                alpha_value = (1.0 - idx/num_trajs) * (max_alpha_value - min_alpha_value) + min_alpha_value
